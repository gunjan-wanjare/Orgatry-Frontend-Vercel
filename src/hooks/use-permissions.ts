...
type Action = 'read' | 'write' | 'delete' | 'export' | 'approve' | 'manage' | 'regularize';
...
function expandScopedEntry(expanded: Set<string>, resource: string, action: Action, scope: Scope) {
  ...
  expanded.add(`${resource}:${action}:${scope}`);
  expanded.add(`${resource}.${action}.${scope}`);
}
...
function expandPermissions(rawPermissions: string[]): Set<string> {
  ...
  for (const permission of rawPermissions) {
    if (permission.includes(':')) {
      ...
      if (parts.length === 3) {
        expandScopedEntry(expanded, parts[0]!, parts[1] as Action, parts[2] as Scope);
      }
      continue;
    }

    const dotParts = permission.split('.').filter(Boolean);
    if (dotParts.length === 3) {
      expandScopedEntry(expanded, dotParts[0]!, dotParts[1] as Action, dotParts[2] as Scope);
    }
  }
...
export function usePermissions() {
  ...
  const hasRequiredPermission = (requiredPermission: string): boolean => {
    ...
    if (requiredPermission.includes('.')) {
      ...
      if (parts.length === 2) {
        ...
        return ['self', 'team', 'department', 'all'].some(
          (scope) =>
  ...
  return {
    ...
    roles,
    can: (permission: string): boolean => hasRequiredPermission(permission),
    ...
    getScope: (resource: string, action: string): Scope => {
      ...
      if (legacyPermissions.has(`${resource}.${action}`) || legacyPermissions.has(`${resource}.read`)) {
        return 'self';
      }
