...
const statusClassName = (status: string, isLate?: boolean) => {
  ...
  if (normalized === "REGULARIZATION_REQUESTED") {
    return "border-blue-500/30 bg-blue-500/10 text-blue-300";
  }
  if (normalized === "REGULARIZATION_APPROVED") {
    return "border-emerald-500/30 bg-emerald-500/10 text-emerald-300";
  }
  if (normalized === "REGULARIZATION_REJECTED") {
    return "border-red-500/30 bg-red-500/10 text-red-300";
  }
  if (normalized === "INCOMPLETE") {
    return "border-violet-500/30 bg-violet-500/10 text-violet-300";
  }
  if (isLate || normalized === "LATE") {
...
const formatDisplayStatusLabel = (record: AttendanceRecord) => {
  if (record.displayStatus) {
    return record.displayStatus.replaceAll("_", " ");
  }
  ...
  return record.isLate ? "LATE" : record.status;
};
...
export function AttendancePage() {
  ...
  const canAccessMyAttendance = canAny([
    permissions.attendanceReadSelf,
    permissions.attendanceReadTeam,
    permissions.attendanceReadDepartment,
  ...
  const canAccessEmployeeAttendance = canAny([
    permissions.attendanceReadTeam,
    permissions.attendanceReadDepartment,
    permissions.attendanceReadAll,
    "attendance:read:team",
  ...
  const canWriteOwnAttendance = canAny([
  ...
  const canRegularizeSelf = canAny([
    permissions.attendanceRegularizeSelf,
  ...
  const analyticsQuery = useQuery({
    queryKey: ["attendance-analytics", "self"],
    queryFn: async () => {
      const now = new Date();
      ...
      const res = await httpClient.get<ApiResponse<AttendanceAnalytics>>(
        `${endpoints.attendanceAnalytics}?scope=self&startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}`,
      );
      return res.data.data;
    },
    enabled: canAccessMyAttendance && !permissionsLoading,
  ...
  const workedMinutes =
    activeSession && activeCheckIn
      ...
      : today?.workedMinutes ?? today?.record?.workedMinutes ?? today?.record?.workMinutes ?? analytics?.todayWorkedMinutes ?? 0;
  const status = today?.status ?? today?.record?.displayStatus ?? today?.record?.status ?? "ABSENT";
  const isLate = today?.isLate ?? today?.record?.isLate ?? false;
  const hasCompletedToday = Boolean(activeCheckIn && activeCheckOut);
  ...
  const regularizableRows = myRecords.map((record) => ({
    ...
    isIncomplete:
      record.isIncomplete ??
      (!(record.checkInTime ?? record.checkInAt) || !(record.checkOutTime ?? record.checkOutAt)),
  }));

  const checkInMutation = useMutation({
    mutationFn: async () => {
      if (activeSession || hasCompletedToday) {
  ...
  const regularizeMutation = useMutation({
    mutationFn: async () => {
      const response = await httpClient.post<ApiResponse<unknown>>(endpoints.attendanceRegularize, {
        ...
        checkInTime: regularizeCheckIn ? new Date(`${regularizeDate}T${regularizeCheckIn}`).toISOString() : undefined,
        checkOutTime: regularizeCheckOut ? new Date(`${regularizeDate}T${regularizeCheckOut}`).toISOString() : undefined,
      });
      return response.data.data;
    },
    onSuccess: () => {
          ...
          actions={
            <div className="flex gap-2">
              ...
              >
                ...
                {activeSession || hasCompletedToday ? "Already Checked In Today" : "Check In"}
              </Button>
              <Button
                variant="outline"
                onClick={() => checkOutMutation.mutate()}
                disabled={!canWriteOwnAttendance || permissionsLoading || checkOutMutation.isPending || !activeSession}
              >
          ...
          <div className="grid gap-4 rounded-xl border border-border/50 bg-white/[0.03] p-4 md:grid-cols-2">
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                <Badge className={statusClassName(status, Boolean(isLate))}>{formatDisplayStatusLabel({ status, isLate, displayStatus: status } as AttendanceRecord)}</Badge>
        ...
        <Dialog open={regularizeOpen} onOpenChange={setRegularizeOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Submit a correction for a single attendance date.</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <div className="grid gap-2">
                <label className="text-sm font-medium">Requested check-in time</label>
              ...
              <div className="grid gap-2">
                <label className="text-sm font-medium">Requested check-out time</label>
            ...
            <DialogFooter>
              ...
              <Button onClick={() => regularizeMutation.mutate()} disabled={regularizeMutation.isPending || !regularizeDate || !regularizeReason || (!regularizeCheckIn && !regularizeCheckOut)}>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {regularizableRows.map((record) => {
                    const canRegularize =
                      canRegularizeSelf &&
                      !record.regularizationRequestId &&
                      (record.isIncomplete ||
                        !(record.checkInTime ?? record.checkInAt) ||
                        !(record.checkOutTime ?? record.checkOutAt));
                    const displayStatus = formatDisplayStatusLabel(record);
                    return (
                      <tr key={record.id} className="border-b border-border/40 last:border-b-0">
                        ...
                        <td className="px-4 py-3">
                          <Badge className={statusClassName(displayStatus, record.isLate)}>{displayStatus}</Badge>
                        </td>
                        ...
                        {employeeRecordsQuery.data.items.map((record) => {
                          ...
                          return (
                            <tr key={record.id} className="border-b border-border/40 last:border-b-0">
                              ...
                              <td className="px-4 py-3">{formatRowDuration(record)}</td>
                              <td className="px-4 py-3">
                                <Badge className={statusClassName(formatDisplayStatusLabel(record), record.isLate)}>
                                  {formatDisplayStatusLabel(record)}
                                </Badge>
                              </td>
                            </tr>
                          );
